...
class IndexController extends Zend_Controller_Action
{
    ...
    {
        if($_SERVER["SERVER_NAME"]=="www.tadikapertiwi.com"){
		header("location:http://tadikapertiwi.com".$_SERVER["REQUEST_URI"]);
	}
        ...
        if(!$lanjut->id){
            ...
            $ud = new Application_Model_DbTable_UserData();
            $us = $ud->tampilNama($lanjut->id);
            $this->view->level = $us->level;
            $ul = new Application_Model_DbTable_UserLevel();
            $ul = $ul->tampilNama($us->level);
            $this->view->levelNama = $ul->nama;
        }
...
//    }
        ...
        if(isset($_POST['simpan']))
	{
            ...
            $user = new Application_Model_DbTable_Masuk();
            $user = $user->ambil($storage->id);
            $kelas = new Application_Model_DbTable_Kelas();
            $kelas = $kelas->tampilNama($user->kelas);
            ...
            if(!trim($judul) || !trim($isi)) $this->view->pesanerror = '<strong>Oalah Cok Ngga Bisa Nyimpen, Soalnya Inputan Kosong.</strong> Tolong Dilengkapi Cok!';            
                ...
                $dbMateri->tambahMateri($kelas->nama, $storage->id, $id, $tanggal, $bulan, $tahun, $mapel, $judul, $isi);
	...
	$storage = new Zend_Session_Namespace();
        ...
        if(isset($_POST['daftar']))
        {
            ...
            if(!trim($nama) || !trim($pengguna) || !trim($katasandi1) || !trim($katasandi2) || !trim($kode) || !trim($email))
            {
                $this->view->pesanerror = "<strong>Sorry Bro, Itu Masih Ada Kosong!</strong> Jadi Harus Dilengkapi Cok!";
            }
            else
            {
                if($kode == "xirpljancok")
...
//        }
    ...
    public function editjadwalAction(){
        ...
        if(isset($_POST["simpan"])){
            ...
            $jadwal = new Application_Model_DbTable_Jadwal();
            $jadwal->updateJadwal($kelas, $senin, $selasa, $rabu, $kamis, $jumat, $sabtu);
        }
    }
    ...
    public function profilAction(){
        ...
        $id = $storage->id;
        if($this->getParam("id") != null && ($this->view->level == 1 || $this->view->level == 2)){
            $id = $this->getParam("id");
        }
    ...
    public function cariAction(){
        ...
        $cari = $this->cari();
        $materi = new Application_Model_DbTable_Materi();
        $this->view->hasil = $materi->cari($cari["judul"], $cari["kelas"], $cari["mapel"]);
    }
    
    public function cari(){
        if(isset($_REQUEST["kata"]) && trim($_REQUEST["kata"]) != ""){
            $key = explode(";", trim($_REQUEST["kata"]));
            $cari["judul"] = $key[0];
            $cari["kelas"] = $key[1];
            $cari["mapel"] = $key[2];
            return $cari;
