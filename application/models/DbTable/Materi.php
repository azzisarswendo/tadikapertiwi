...
class Application_Model_DbTable_Materi extends Zend_Db_Table_Abstract
{
    ...
    public function tambahMateri($kelas, $id_user, $id_materi, $tanggal, $bulan, $tahun, $mapel, $judul, $isi)
    {
        $data = array(
            'kelas' => $kelas,
            'user' => $id_user,
    ...
    public function cari($judul, $kelas, $mapel){
        $select= $this->select();
        $select->where("judul like '%$judul%' and kelas like '%$kelas%' and mapel like '%$mapel%'");
